...
//hapus transaksi
  ...
  mysql_query("DELETE FROM trans_tujuan WHERE no_do=(SELECT no_do FROM transaksi WHERE no_trans='$_GET[id]')"); 
...
//hapus DO
 ...
 mysql_query("DELETE FROM trans_do WHERE no_do='$no_do'"); 
  mysql_query("DELETE FROM trans_tujuan WHERE no_do='$no_do'"); 
 mysql_query("update transaksi set multidrop_klien='multi1', total_klien='$total1', multidrop_vendor='multi2', total_vendor='$total2', margin='$margin' where no_trans='$no_trans'");
...
//edit DO
  ...
  $qty=$_GET['qty'];
  mysql_query("UPDATE trans_do set no_load='$no_load', no_shipment='$no_ship', kuantiti='$qty' WHERE no_do='$no_do'"); 
...
$no_ship=$_GET['no_ship'];
$qty=$_GET['qty'];
$cekdata="SELECT no_do FROM trans_do WHERE no_do='$no_do'"; 
$ada=mysql_query($cekdata) or die(mysql_error()); 
if(mysql_num_rows($ada)>0) { 
echo "<script type='text/javascript'> alert('No DO sudah ada, silakan ulangi kembali');</script>";
} else { 
if (!empty($no_trans) AND !empty($no_do) AND !empty($no_load) AND !empty($no_ship) AND !empty($qty) ) {
