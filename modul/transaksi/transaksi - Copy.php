        ...
        <body>
            ...
            switch($p){
                default:
					...
					$tampil=mysql_query("select * from transaksi, tbl_klien where tbl_klien.id_klien = transaksi.id_klien order by no_trans LIMIT $posisi,$batas");
				  ...
				  echo "<tr>
		            ...
		            <td width=150><span><a href=?module=transaksi&act=detail&id=$dt[no_trans]>Detail | </a></span><span><a href='?module=edit_transaksi&id=$dt[no_trans]'>Edit | </a></span>";
					echo"<span><a href=$aksi?module=transaksi&act=hapus&id=$dt[no_trans] onClick=\"return confirm('Yakin transaksi No $dt[no_trans] akan di hapus?')\">Hapus | </a></span><span><a href=\"\" onClick=\"return alert('$pesan')\">View</a></span></td>
				  </tr>";
				  $no++;
				  }
...
echo"			<a href=?module=transaksi&act=edit_do&id=$s[no_do]><b>Edit | </b></a><a href=?module=transaksi&act=hapus_do&id=$s[no_do]><b>Hapus</b></a>";	
		...
		$multi2=mysql_fetch_array(mysql_query("select * from multidrop2 where tipe_mobil='$r[tipe_mobil]' and kode_vendor=$r[kode_vendor]"));
			...
			echo"<tr><td colspan=5 align=right><b>Total Pengiriman : </b></td><td colspan=2><b>$tot[jumlah] Titik</b></td></tr><tr><td colspan=5 align=right><b>Multidrop : </b></td><td colspan=2><b>$multi[jumlah]</b></td></tr><tr><td colspan='7' align='right'><input type=button onclick=\"window.location.href='?module=transaksi&act=tambah_do&id=$r[no_trans]';\" value='Tambah DO'></td>
			  </table>
