...
*/	
                ...
                $(function(){
					...
					$("#edit_do").click(function(){
						...
						var trans_do_id=$("#trans_do_id").val();
						var no_do=$("#no_do").val();
						var no_load=$("#no_load").val();
						var no_ship=$("#no_ship").val();
						var qty=$("#qty").val();
                        $.ajax({
                            url:"modul/transaksi/proses.php",
                            data:"op=edit_do&trans_do_id="+trans_do_id+"&no_do="+no_do+"&no_load="+no_load+"&no_ship="+no_ship+"&qty="+qty,
                            cache:false,
                            success:function(msg){
									self.history.back();
                            }
					...
					$("#hapus").click(function(){
                        ...
                        $.ajax({
                            url:"modul/transaksi/proses.php",
                       data:"op=hapus_do&trans_do_id="+trans_do_id+"&no_do="+no_do+"&no_trans="+no_trans+"&harga_klien="+harga_klien+"&harga_vendor="+harga_vendor+"&uj="+uj+"&gaji="+gaji+"&sewa="+sewa+"&multi="+multi+"&multi_klien="+multi_klien+"&multi_vendor="+multi_vendor,
                            cache:false,
                            success:function(msg){
									self.history.back();
                            }
...
?>
            ...
            switch($p){
                default:
					...
					$tampil=mysql_query("SELECT tr.no_trans,k.nama_klien,td.no_load,tr.total_klien,tr.tanggal,tr.updated,tr.updated_by,tr.created,tr.created_by FROM (transaksi tr LEFT JOIN trans_do td ON tr.no_trans = td.no_trans), tbl_supir s, zona z, tbl_klien k WHERE s.id_supir=tr.id_supir AND k.id_klien=tr.id_klien AND z.kode_zona=tr.kode_zona");
				  ...
				  while($dt=mysql_fetch_array($tampil)){
					$harga=number_format($dt[total_klien],0,",",".");
					$cb=$dt[created_by];
					$co=$dt[created];
					$ub=$dt[updated_by];
					$uo=$dt[updated];
				  ...
				  echo "<tr>
					...
					<td>$dt[nama_klien]</td>
					<td width=100 align=center>$dt[no_trans]</td>
					<td align=center>$dt[no_load]</td>
					<td width=auto align=center>";
					   list($thn,$bln,$tgl)=explode('-',$dt[tanggal]);
   						echo $tgl.' '.$namabulan[(int)$bln].' '.$thn;
					echo "</td>
				    <td align=center>$harga</td>
		            <td width=150><span><a href=?module=transaksi&act=detail&id=$dt[no_trans]>Detail | </a></span><span><a href='?module=edit_transaksi&id=$dt[no_trans]'>Edit | </a></span><span><a href=$aksi?module=transaksi&act=hapus&id=$dt[no_trans] onClick=\"return confirm('Yakin transaksi No $dt[no_trans] akan di hapus?')\">Hapus | </a></span><span><a href=\"\" onClick=\"return alert('$pesan')\">View</a></span></td>
				  </tr>";
				  $no++;
				  }
...
//					echo "</table><br/><p align=center>Hal $linkHalaman</p>";
				...
				echo "<tr><td>No Transaksi</td><td>: <input type=text name='notrans' id='notrans' placeholder='No Transaksi' readonly></td></tr>
				...
				<tr><td>Tipe Mobil</td><td>: <input type=text name='tipe_mobil' id='tipe_mobil' placeholder='Tipe Mobil' readonly></td></tr>
				...
				<tr><td>Harga ke Klien</td><td>: <input type=text name='harga1' id='harga1' class='nominal'  placeholder='Harga Klien' readonly/></td></tr>
				<tr><td>Harga ke Vendor</td><td>: <input type=text name='harga2' id='harga2' class='nominal'  placeholder='Harga Vendor' readonly/></td></tr>
				<tr><td>Uang Jalan</td><td>: <input type=text name='uj' id='uj' class='nominal' placeholder='Uang Jalan'/></td></tr>
				<tr><td>Gaji Driver</td><td>: <input type=text name='gaji' id='gaji' class='nominal' placeholder='Gaji Driver'/></td></tr>
				<tr><td>Sewa Mobil</td><td>: <input type=text name='sewa' id='sewa' class='nominal' placeholder='Sewa Mobil'/></td></tr>
				<tr><td>Keterangan</td><td>: <textarea id='ket' style='width: 30%' cols='200' rows='3' name='ket' placeholder='Keterangan'></textarea></td></tr>
				</table>
				</td>
				</tr>
			...
			<tr><th width=50>No</th><th>No Load</th><th>No DO</th><th>No Shipment</th><th>Kuantiti</th><th>Jumlah Tujuan</th><th>Aksi</th></tr>";
			$no=1;
			while($s= mysql_fetch_array($q)){		  
			$q2 = mysql_fetch_array(mysql_query("SELECT count(tujuan) as jumlah FROM trans_tujuan where trans_do_id='$s[trans_do_id]'"));
		...
		$jumlah_multi=$multi_all;
			...
			<td width=100><a href=?module=detail_do&id=$s[trans_do_id]><b>Detail | </b></a>
			<a href=?module=transaksi&act=edit_do&id=$s[trans_do_id]><b>Edit | </b></a><a href=?module=transaksi&act=hapus_do&id=$s[trans_do_id]><b>Hapus</b></a>";	
			$no++;	  
			}		
		...
		case "edit_do" :
		$edit=mysql_query("select * from trans_do where trans_do_id='$_GET[id]'");
		$data=mysql_fetch_array($edit);
		echo "<h2 class=head>Detail Transaksi Pengiriman</h2>
		<table class=tabelform><form>
		<span id='status'></span>
		<input type='hidden' name='trans_do_id' id='trans_do_id' value='$data[trans_do_id]' >
		...
		<tr><td>No DO</td><td>: <input type='text' name='no_do' id='no_do' value='$data[no_do]'></td></tr>
		...
		$edit=mysql_query("select * from trans_do where trans_do_id='$_GET[id]'");
		$edit2=mysql_query("select count(*) as total, kode_tujuan, tujuan from trans_tujuan where trans_do_id='$_GET[id]'");
		$data=mysql_fetch_array($edit);
		...
		<input type=hidden value='$data[trans_do_id]' name='trans_do_id' id='trans_do_id'>
		<tr><td>No Transaksi</td><td>: <input type='text' name='no_trans' id='no_trans' value='$data[no_trans]' readonly></td></tr>
